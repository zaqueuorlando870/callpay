...
namespace Botble\Callpay\Services\Abstracts;

use Botble\Payment\Services\Traits\PaymentErrorTrait;
use Botble\Callpay\Services\Callpay;
use Botble\Support\Services\ProduceServiceInterface;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Arr;

abstract class CallpayPaymentAbstract implements ProduceServiceInterface
{
    ...
    public function getPaymentDetails($payment)
    {
        try {
            ...
            $response = (new Callpay())->getListTransactions($params);
            if ($response['status']) {
    ...
    {
        try {
            $response = (new Callpay())->refundOrder($paymentId, $amount);
    ...
    {
        try {
            $response = (new Callpay())->getRefundDetails($refundId);
            if ($response['status']) {
                return [
                    'error' => false,
                    'message' => $response['message'],
